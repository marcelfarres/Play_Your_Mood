...
void compute(const string& audioFilename, const string& outputFilename, const string& modelsPath, Pool& pool, const Pool& options);
...
void addSVMDescriptors(Pool& pool, const string& modelsPath);
...
int main(int argc, char* argv[]) {
  ...
  string audioFilename, outputFilename, modelsPath;
  ...
  try {
    compute(audioFilename, outputFilename, modelsPath, pool, options);
  }
...
#if HAVE_GAIA2
  addSVMDescriptors(stats, modelsPath);
#else
...
void addSVMDescriptors(Pool& pool, const string& modelsPath) {
  ...
  // msg << modelsPath << "svm_models/";
  // pathToSvmModels = msg.str();
  // cout << pathToSvmModels << endl;
